...
const Login = () => {
  ...
  // Set form context and redirect authenticated users to dashboard
  useEffect(() => {
    setCurrentForm('login');
    if (user && !isLoading) {
      navigate('/dashboard', { replace: true });
    }
  }, [user, isLoading, navigate, setCurrentForm]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Welcome back!",
        description: "You've successfully logged in to FinSight.",
      });
    } catch (error) {
      toast({
        variant: "destructive",
        ...
        <Card className="card-floating">
          ...
          <CardContent>
            ...
            <Button
              type="button"
              variant="outline"
              className="w-full font-light text-lg py-6"
            ...
            <div className="mt-6 text-center">
              <Link 
                to="/forgot-password" 
                className="text-sm text-primary hover:text-primary-glow font-light transition-smooth"
              >
